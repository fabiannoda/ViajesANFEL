...
public class modelAgente {
    ...
    private ResultSet EncontrarCliente(String idRegistro) throws SQLException {
        String sql = "select nombreAgente, apellidoAgente, fechaNacimientoAgente, edadAgente, direccionAgente, correoAgente, telefonoAgente from agenteDeViajes where agenteDeViajes.idAgente=?";
    ...
    private ResultSet fill() throws SQLException {
        conexion cc = new conexion();
        Connection cn = cc.conex(Controlador.ControladorMenu.user, Controlador.ControladorMenu.pass);
        String sql = "SELECT idAgente, nombreAgente, apellidoAgente, fechaNacimientoAgente, edadAgente, direccionAgente, correoAgente, telefonoAgente FROM Anfel.agenteDeViajes";
        PreparedStatement pst = cn.prepareStatement(sql);
    ...
    public void insertar() {
        ...
        sql = "insert into agenteDeViajes (idAgente, nombreAgente, apellidoAgente, fechaNacimientoAgente, edadAgente, direccionAgente, correoAgente) values (?,?,?,?,?,?,?,?)";
        try {
            PreparedStatement pst = cn.prepareStatement(sql);
            pst.setString(1, id);
            pst.setString(2, nom);
    ...
    public void actualizar(String ident) {
        ...
        corre = client.txtcorr.getText();
        tel = client.txttel.getText();
        sql = "update agenteDeViajes set nombreAgente='" + nom + "', apellidoAgente='" + ape + "', fechaNacimientoAgente='" + fecha + "', edadAgente='" + edad + "', direccionAgente='" + dire + "', correoAgente='" + corre + "',telefonoAgente='" + tel + "' where idAgente='" + ident + "';";
        try {
            PreparedStatement pst = cn.prepareStatement(sql);
            int registro = pst.executeUpdate();
            if (registro > 0) {
                JOptionPane.showMessageDialog(null, "Actualización exitosa del Agente No. " + ident + "");
                limpiar();
                bloquear();
            } else {
    ...
    public void eliminar(String ident) {
        ...
        Connection cn = cc.conex(Controlador.ControladorMenu.user, Controlador.ControladorMenu.pass);
        String sql = "";
        sql = "delete from agenteDeViajes where "
                + "idAgente= '" + ident + "'";
        try {
            PreparedStatement pst = cn.prepareStatement(sql);
            int registro = pst.executeUpdate();
            if (registro > 0) {
                JOptionPane.showMessageDialog(null, "Agente No. " + ident + " eliminado exitosamente");
                limpiar();
                bloquear();
            } else {
                JOptionPane.showMessageDialog(null, "Agente no encontrado");
