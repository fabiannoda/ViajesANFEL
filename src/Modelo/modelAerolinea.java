 ...
 * @author USER
 */
public class modelAerolinea {
    public aerolinea vista;
    ...
    private ResultSet fill() throws SQLException{
        ...
        String sql="SELECT idAerolinea,nombreAerolinea FROM Anfel.aerolinea";
    ...
    public void insert(){
        ...
        try {
            ...
            if (registro>0) {
                JOptionPane.showMessageDialog(null,"Aerolinea registrada exitosamente");
                limpiar();
            }
        } catch (Exception e) {
    ...
    public void actuali(String id){
        ...
        String cade,sql="";
        cade=vista.txtnom.getText();
        sql="update aerolinea set nombreAerolinea='"+cade+"' where idAerolinea='"+id+"';";
        try {
            PreparedStatement pst=cn.prepareStatement(sql);
            int registro=pst.executeUpdate();
            if (registro>0) {
                JOptionPane.showMessageDialog(null, "Actualización exitosa de la aerolinea No. "+id+"");
                limpiar();
            }else{
    ...
    public void eliminar(String id){
        ...
        Connection cn=cc.conex(Controlador.ControladorMenu.user, Controlador.ControladorMenu.pass);
        String sql="";
        sql="delete from aerolinea where "
                +"idAerolinea= '"+id+"';";
        try {
            PreparedStatement pst=cn.prepareStatement(sql);
            int registro=pst.executeUpdate();
            if (registro>0) {
                JOptionPane.showMessageDialog(null, "Aerolinea No. "+id+" eliminado exitosamente");
                limpiar();
